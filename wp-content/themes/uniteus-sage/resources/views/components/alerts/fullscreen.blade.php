@if (isset($display))
    ...
    @if (in_array($post->ID, $display) or empty($display))
        ...
        <section x-data="{ hideAlert: localStorage.getItem('hideUuGlobalAlert') === '{{ $unique_id }}' }" x-bind:class="{ 'hidden': hideAlert == true }"
            class="relative component-section alert !p-0"
            style="@isset($text_color) color: {{ $text_color }}; @endisset @isset($background_color) background: {{ $background_color }}; @endisset">
                        ...
                        <div class="w-full flex md:items-center gap-4">
                            <div class="flex flex-shrink-0 pt-1">
                                ...
                                @if ('custom' == $style)
                                    @isset($icon)
                                        <img class="h-6 w-6"
                                            src="/wp-content/themes/uniteus-sage/resources/icons/acf/{{ $icon }}.svg"
                                            alt="" />
                                    @endif
        @endif
        ...
        <div class="flex-shrink-0 flex items-center absolute top-0 right-0 mt-4 mr-4 sm:m-0 sm:relative">
            <svg @click="localStorage.setItem('hideUuGlobalAlert', '{{ $unique_id }}'); hideAlert = true"
                xmlns="http://www.w3.org/2000/svg" width="25" height="24" viewBox="0 0 25 24" fill="none">
                <path d="M6.5 18L18.5 6M6.5 6L18.5 18" stroke="white" stroke-width="2" stroke-linecap="round"
