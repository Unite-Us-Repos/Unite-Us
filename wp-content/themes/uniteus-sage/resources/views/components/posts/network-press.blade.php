...
<section
    ...
    <div class="component-inner-section relative z-20 @if ($section_settings['fullscreen']) fullscreen @endif">
            ...
            <div>
                @if ($section['title'] || $section['description'])
                    <div class="text-center max-w-4xl mx-auto @if ($background['color'] == 'dark') text-white @endif">
                        @if ($section['subtitle'])
                            <div class="text-action-light-blue uppercase font-semibold mb-4">{{ $section['subtitle'] }}
                            </div>
                        @endif
                        @if ($section['title'])
                            <h2>{{ $section['title'] }}</h2>
        ...
        @if ($press)
            <div id="network-press"
                class="mt-10 mx-auto grid gap-8 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 lg:max-w-none">
                @foreach ($press as $index => $post)
                    @php
                        $is_hidden = $index >= $ppp ? 'hidden' : '';
                        $post_cat = '';
                    ...
                    @endphp
                    <div
                        class="relative flex flex-col bg-white rounded-lg shadow-lg overflow-hidden {{ $is_hidden }}">
                        @if ($thumbnail_url)
                            <div class="flex-shrink-0 related-gradient border-b-2 border-light">
                                <img class="aspect-video w-full @if ($default_image) object-contain p-8 px-14 @else object-cover @endif mx-auto"
                                    src="{{ $thumbnail_url }}" alt="{{ $thumbnail_alt }}">
                            </div>
                        @endif
                        <div class="flex-1 bg-white flex flex-col justify-between">
                            <div class="flex-1 px-6 pt-7 pb-10">
                                @isset($post_cat)
                                    <a href="{{ $post_cat_link }}"
                                        class="leading-normal inline-block text-sm font-medium text-action mb-2">
                                        <span
                            ...
                            <div class="bg-light hover:bg-blue-200">
                                @if ($post['permalink'])
                                    <a class="no-underline text-action flex gap-3 items-center font-semibold p-6"
                                        href="{{ $post['permalink'] }}"
                                        @if ($post['is_external']) target="_blank @endif">Read More<span
                                ...
                                @else
                                    <span class="p-6 block">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="11"
                                            ...
                                            <path d="M7.875 1.5105L12 5.6355M12 5.6355L7.875 9.7605M12 5.6355L1 5.6355"
            ...
            @if (count($press) > $ppp)
                <div class="mt-14 text-center">
                    <button type="button" class="inline-flex button button-solid load-more-button"
                        data-total-posts="{{ count($press) }}" data-visible-posts="{{ $ppp }}">
                        <span class="mr-4 inline-block">Load More</span>
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path class="path-hover" fill-rule="evenodd" clip-rule="evenodd"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let visiblePosts = parseInt(loadMoreButton.getAttribute('data-visible-posts'));
        const totalPosts = parseInt(loadMoreButton.getAttribute('data-total-posts'));

        loadMoreButton.addEventListener('click', function() {
            const hiddenPosts = document.querySelectorAll('#network-press > div.hidden');
