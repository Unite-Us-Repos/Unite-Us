...
<section id="{{ $section['id'] }}"
    ...
    @php
        ...
        // Convenience vars
        $firstCat = (!empty($pressCats) && !is_wp_error($pressCats)) ? $pressCats[0] : null;

        // Hide image if ANY press_cat term is 'company-news'
        $pressCatSlugs = (!empty($pressCats) && !is_wp_error($pressCats)) ? wp_list_pluck($pressCats, 'slug') : [];
        $isCompanyNews = in_array('company-news', $pressCatSlugs, true);
    @endphp
        ...
        <div class="relative flex flex-col overflow-hidden bg-transparent">
            <div class="flex-1 flex flex-col justify-between">
                <div class="flex-1 pt-7 pb-4">
                    ...
                    <h3 class="mb-4">
                        @if ($post['permalink'])
                            <a class="no-underline text-white" href="{{ $post['permalink'] }}" @if ($post['is_external']) target="_blank" @endif>
                        @endif
                        {!! $post['post_title'] !!}
                        @if ($post['permalink'])
                            </a>
                    ...
                    @if ($post['permalink'])
                        <a class="no-underline light-blue font-semibold p-6 pt-4 pl-0 block"
                           href="{{ $post['permalink'] }}" @if ($post['is_external']) target="_blank" @endif>
        ...
        <div class="relative flex flex-col rounded-lg shadow-lg overflow-hidden bg-white">
            ...
            <div class="flex-1 flex flex-col justify-between">
                <div class="flex-1 px-6 pt-7 pb-10">
                    ...
                    <h3 class="mb-4">
                        @if ($post['permalink'])
                            <a class="no-underline text-brand" href="{{ $post['permalink'] }}" @if ($post['is_external']) target="_blank" @endif>
                        @endif
                        {!! $post['post_title'] !!}
                        @if ($post['permalink'])
                            </a>
