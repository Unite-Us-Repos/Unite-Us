...
<section class="hello component-section {{ $section_classes }} @if ($section_settings['collapse_padding']) {{ $section_settings['padding_class'] }} @endif">
    <div class="component-inner-section @if ($section_settings['fullscreen']) fullscreen @endif">
              ...
              @if ($section['title'] || $section['description'])
                    <div class="relative pt-20 pb-10 mb-10 z-10">
                        <div class="text-whitetext-center max-w-5xl mx-auto">
                            @if ($section['title'])
                                <h{{ $h_level }}
                                    @if ('1' === $h_level) class="text-5xl lg:text-6xl mb-10" @endif>
                                    {!! $section['title'] !!}</h{{ $h_level }}>
                            @endif
                ...
                <div id="ajax-spotlight" class="mt-10 max-w-lg mx-auto grid gap-8 lg:grid-cols-2 xl:grid-cols-3 lg:max-w-none">
                    @foreach ($posts as $index => $post)
                        @php
                            ...
                            $event_info = get_field('event_info', $post['ID']);

                            // Extract event dates and times
                            $event_date = $event_info['event_dates']['start'] ?? 'TBA';
                            $end_date = $event_info['event_dates']['end'] ?? '';

                            // Convert event_date from YYYYMMDD to readable format
                            if ($event_date !== 'TBA') {
                                $event_date = date('F j, Y', strtotime($event_date));
                            }

                            if (!empty($end_date) && $end_date !== $event_info['event_dates']['start']) {
                                $end_date = date('F j, Y', strtotime($end_date));
                                $event_date .= ' - ' . $end_date;
                            }

                            // Extract start and end times
                            $start_time = $event_info['start_time'] ?? 'TBA';
                            $end_time = $event_info['end_time'] ?? '';

                            // Format time output
                            $formatted_time = !empty($end_time)
                                ? "{$start_time} — {$end_time}"
                        ...
                        <div class="relative group flex flex-col rounded-lg shadow-lg overflow-hidden">
                            @if ($post['permalink'])
                                <a class="absolute inset-0 z-40" class="no-underline text-brand"
                                    href="{{ $post['permalink'] }}"
                            ...
                            @isset($post['thumbnail_url'])
                                <div class="flex-shrink-0 bg-white border-b-2 border-light z-10">
                                    <img class="aspect-[5/2] w-full object-cover mx-auto" src="{{ $post['thumbnail_url'] }}"
                                        ...
                                        <div
                                            ...
                                            <span>
                                                  <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                                      ...
                                                      <path fill-rule="evenodd" clip-rule="evenodd"
                                                          d="M10.0014 18C14.1987 18 17.6014 14.4183 17.6014 10C17.6014 5.58172 14.1987 2 10.0014 2C5.804 2 2.40137 5.58172 2.40137 10C2.40137 14.4183 5.804 18 10.0014 18ZM10.9514 6C10.9514 5.44772 10.526 5 10.0014 5C9.47669 5 9.05136 5.44772 9.05136 6V10C9.05136 10.2652 9.15145 10.5196 9.32961 10.7071L12.0166 13.5355C12.3876 13.9261 12.9891 13.9261 13.3601 13.5355C13.7311 13.145 13.7311 12.5118 13.3601 12.1213L10.9514 9.58579V6Z"
                                                          fill="#216CFF"></path>
                                                  </svg>
                                            </span>
                                            <span class="text-brand text-sm">{{ $formatted_time }}</span>
                                        </div>
        ...
        @if ($total_press > $ppp)
            <div class="mt-14 text-center">
                <button type="button" class="inline-flex button button-solid load-more-button loadmore-posts"
                    data-post-type="1c" data-ajax-container="ajax-spotlight" data-ppp="{{ $ppp }}"
                    data-current-page="1" data-template="1c-event" data-press-cat="{{ $category }}">
                    <span class="mr-4 inline-block">Load More</span>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd"
