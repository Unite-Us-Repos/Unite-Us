...
@php
    ...
    $section_settings = isset($data['recommended_pages']['section_settings'])
        ? $data['recommended_pages']['section_settings']
        : $section_settings;
            ...
            @if ($cards)
                <div class="flex flex-col flex-wrap lg:justify-center gap-y-6 sm:flex-row">
                    @foreach ($cards as $index => $card)
                        ...
                        @endphp
                        <div
                            class=" md:basis-6/12 @if ($columns) lg:basis-{{ $columns }} @else sm:basis-2/6 @endif pt-6">
                            ...
                            <div class="relative h-full test px-5">
                                    ...
                                    @isset($card['icon'])
                                        @if (!empty($card['icon']))
                                            @if ('round' == $card['icon_type'])
                                                <span
                                                    class="inline-flex w-12 h-12  items-center justify-center rounded-full border-4 border-action bg-brand p-2 shadow-lg">
                                                @else
                                                    <span
                                                        class="inline-flex items-center justify-center rounded-md bg-action p-3 shadow-lg">
                                            @endif
                                            <img class="h-8 w-8"
                                                src="/wp-content/themes/uniteus-sage/resources/icons/acf/{{ $card['icon'] }}.svg"
                                                alt="" />
                                            </span>
                                        @endif
                                    @endisset
                                        ...
                                        @if (!empty($card['custom_icon']))
                                            <span class="inline-flex items-center">
                                                <img class="h-20 w-20 object-contain"
                                                    src="{{ $card['custom_icon']['sizes']['medium'] }}"
                                                    alt="" />
                                            </span>
                                        @endif
                                    @endif
                                ...
                                @if ($link)
                                    <a class="no-underline text-action font-semibold block" href="{{ $link }}"
