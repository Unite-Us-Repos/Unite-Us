            ...
            <div class="flex flex-col gap-6 mt-6">
              @foreach ($blog_card as $card)
                @php
                  ...
                  $category = (!empty($categoryObjs)) ? ($categoryObjs[0]->cat_name ?? '') : '';
                @endphp
          ...
          @foreach ($testimonials['testimonials'] as $t)
            <blockquote class="!p-0 !border-none !not-italic @if ($loop->index == 0) lg:mt-44 @endif">
              <div class="flex items-start mt-6 gap-6">
                @isset($t['image']['sizes'])
                  <img class="w-16 h-16 object-contain rounded-full" src="{{ $t['image']['sizes']['thumbnail'] }}" alt="{{ $t['image']['alt'] }}" />
                @endisset
                <div>
                  <div class="text-lg text-brand mb-6">{!! $t['quote'] !!}</div>
                  <p class="text-brand text-base">
                    <span class="font-bold">{!! $t['name'] !!}</span>
                    @if (!empty($t['title'])), <span class="text-action text-base font-semibold">{!! $t['title'] !!}</span>@endif
                  </p>
                </div>
              </div>
            </blockquote>
