...
<section @isset($section['id']) id="{{ $section['id'] }}" @endisset
    ...
    <div class="relative z-10 component-inner-section @if ($section_settings['fullscreen']) fullscreen @endif">
        <div class="md:relative flex flex-col lg:grid lg:grid-cols-12 lg:gap-28">
            ...
            @isset($extra_content)
                        ...
                        @if ($testimonial['style'] == 'inline')
                            <blockquote class="!p-0 !border-none !not-italic">
                                ...
                                <div class="flex items-center mt-6 gap-3">
                                    ...
                                    <p class="text-brand text-base"><span class="font-bold">{!! $testimonial['name'] !!}</span>
                        ...
                        @else
                            <blockquote class="!p-0 !border-none">
                                ...
                                <p class="text-brand font-semibold text-base text-center not-italic mt-3">
                                    {!! $testimonial['name'] !!}@if ($testimonial['title'])
                                        , {!! $testimonial['title'] !!}
                                    @endif
                                </p>
                            </blockquote>
                ...
                @if ('mini-cards' == $extra_content)
                    @if (!empty($mini_cards))
                        @php
                            $count = is_countable($mini_cards) ? count($mini_cards) : 0;
                        @endphp
                        ...
                        <div class="flex flex-col gap-6 mt-6">
                            @foreach ($blog_card as $card)
                                ...
                                <div x-data="" class="relative group rounded-xl overflow-hidden">
                                    <div
                                        class="absolute p-7 flex justify-end inset-0 z-10 rounded-xl group-hover:opacity-0">
        ...
        <div
            class="flex flex-col lg:max-h-[600px] @if ('accordion' == $type) lg:col-span-7 lg:col-end-13 @else lg:col-span-{{ $columns[1] }} @endif @if ('center' == $vertical_alignment) justify-center @endif relative @if ('image' == $type) @if ('text_image' == $layout) lg:order-2 @else  lg:order-1 @endif @endif">
            ...
            @if ('image' == $type)
                @if ($mask_image)
                    ...
                    margin-top: 17%;
                    margin-left: 20%;
                    @if (!$video) background: url({{ $featured_image['sizes']['medium_large'] }}) no-repeat center center;background-size: cover; @endif
                    ">
                                @if ($video)
                ...
                @else
                    @isset($featured_image['sizes'])
                        <img class="lazy mb-6 lg:mb-0 rounded-lg w-full @if ($set_max_width_height) h-full object-contain max-w-lg mx-auto @else max-w-md mx-auto lg:max-w-3xl @endif"
            ...
            @if ('video' == $type)
                @if ($video)
                    <video autoplay loop muted playsinline poster="{{ $featured_image['sizes']['medium_large'] }}"
                        ...
                        <source data-src="{{ $video }}" type="video/mp4" />Your browser does not support the
                        video tag.
                    </video>
                @endif
            @endif
                            ...
                            @foreach ($accordion as $index => $card)
                                <li class="relative social-card py-6 px-9 lg:p-10 mb-6 bg-white rounded-lg shadow-lg"
                                    ...
                                    <div class="relative overflow-hidden transition-all max-h-0 duration-700"
                                        style="" x-ref="container{{ $index }}"
                                        x-bind:style="selected == {{ $index }} ? 'max-height: ' + $refs
                                ...
                                @if (isset($item['svg_picker_or_custom_icon_upload']))
                                    <div class="basis-1/5 flex justify-start lg:justify-end align-middle">
                                        @if ($item['svg_picker_or_custom_icon_upload'] == 'SVG Icon Picker' && !empty($item['icon']))
                                            <span
                                                class="icon w-10 h-10 p-2 flex justify-center items-center rounded-full mb-5 bg-light ">
                                                <img class="lazy h-full w-full"
                                                    data-src="/wp-content/themes/uniteus-sage/resources/icons/acf/{{ $item['icon'] }}.svg"
                                                    alt="" />
                                            </span>
                                        @elseif ($item['svg_picker_or_custom_icon_upload'] == 'Custom' && !empty($item['icon_custom_upload']))
                                            <span
                                                class="icon w-10 h-10 flex justify-center items-center rounded-full bg-light ">
                                                <img src="{{ $item['icon_custom_upload']['url'] }}"
                                                    alt="{{ $item['icon_custom_upload']['alt'] }}"
                                                    class="custom-icon w-10 h-10">
                                            </span>
                                        @endif
                                    </div>
                                @endif
...
@if ('testimonials' == $extra_content)
    <section class="component-section padding-collapse-t -mb-4"
        style="background: url(/wp-content/uploads/2024/01/spider-webs.png) no-repeat center center;
                        ...
                        <blockquote
                            ...
                            <div class="flex items-start mt-6 gap-6">
                                ...
                                <div>
                                    ...
                                    <div class="text-brand text-base"><span
                                            class="font-bold">{!! $testimonial['name'] !!}</span>
                                        @if ($testimonial['title'])
                                            ,
                                            <div class="text-action text-base font-semibold">{!! $testimonial['title'] !!}
                                            </div>
                                        @endif
                                    </div>
                                </div>
