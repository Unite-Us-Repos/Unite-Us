<style>
  .mini-card-grad{
    background-color:#E3EBF3; /* fallback */
  ...
  <div class="relative z-10 component-inner-section @if (!empty($section_settings['fullscreen'])) fullscreen @endif">
    <div class="md:relative flex flex-col lg:grid lg:grid-cols-12 lg:gap-14">
      ...
      <div class="flex flex-col items-start
                  @if ($type === 'accordion') lg:col-span-4 @else lg:col-span-{{ $colA }} @endif
                  @if ($vertical_alignment === 'center') justify-center @endif
                  @if ($type === 'image' || $type === 'embed') order-2 @endif
                  text-lg
                  @if ($type === 'image')
                    @if ($layout === 'text_image') lg:order-1 @else lg:order-2 @endif
                  @endif">
        ...
        @isset($extra_content)
              ...
              <blockquote class="!p-0 !border-none !not-italic">
                ...
                <div class="flex items-center mt-6 gap-3">
                  ...
                  <p class="text-brand text-base">
                    ...
                    @if (!empty($testimonial['title'])), <span class="text-action font-semibold">{!! $testimonial['title'] !!}</span>@endif
                  </p>
                </div>
              </blockquote>
            @else
              <blockquote class="!p-0 !border-none">
                ...
                <p class="text-brand font-semibold text-base text-center not-italic mt-3">
                  {!! $testimonial['name'] !!}
                  @if (!empty($testimonial['title'])), <span class="text-action">{!! $testimonial['title'] !!}</span>@endif
                </p>
              </blockquote>
            @endif
          @endif
          ...
          @if ($extra_content === 'mini-cards' && !empty($mini_cards))
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 {{ $count > 4 ? 'lg:grid-cols-3' : 'lg:grid-cols-2' }} gap-3 mt-4">
              @foreach ($mini_cards as $card)
                <div class="relative text-center sm:text-left border border-blue-200 rounded-xl p-6 group overflow-hidden">
                  <div class="relative z-10">
                    <h2 class="font-extrabold text-3xl gradient-text mb-0 leading-10 w-fit">{!! $card['title'] !!}</h2>
                    <div class="text-base">{!! $card['description'] !!}</div>
                  </div>
                  <div class="absolute inset-0 rounded-xl mini-card-grad z-0 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity"></div>
                </div>
              @endforeach
            </div>
          @endif
                ...
                <div x-data class="relative group rounded-xl overflow-hidden">
                  ...
                  <div @click.prevent="window.location.href='{{ get_permalink($card->ID) }}'"
                    ...
                    <div class="sm:col-span-8 p-6">
                      ...
                      <h3 class="@if ($blog_card_style === 'light') text-brand @else text-white @endif mb-3 font-semibold">
                        {!! $card->post_title !!}
                      </h3>
                      ...
                      @if ($blog_card_content === 'title_more')
                        <a class="card-link @if ($blog_card_style === 'light') text-electric-purple group-hover:text-action-light @else text-action-light-blue group-hover:text-action @endif font-semibold no-underline flex flex-row items-center gap-3"
                          ...
                          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="11" viewBox="0 0 14 11" fill="none">
                            <path d="M8.66016 1.09241L12.7852 5.21741M12.7852 5.21741L8.66016 9.34241M12.7852 5.21741L1.78516 5.21741"
      ...
      <div class="flex flex-col lg:max-h-[600px]
                  @if ($type === 'accordion') lg:col-span-7 lg:col-end-13 @else lg:col-span-{{ $colB }} @endif
                  @if ($vertical_alignment === 'center') justify-center @endif
                  relative
                  @if ($type === 'image')
                    @if ($layout === 'text_image') lg:order-2 @else lg:order-1 @endif
                  @endif">

        @if ($type === 'image')
          @if (!empty($mask_image))
            @if (!empty($featured_image))
              @php if (!empty($image_mask)) { $image_overlay = $image_mask; } @endphp
              ...
              @isset($accordion)
                @foreach ($accordion as $i => $card)
                  <li class="relative social-card py-6 px-9 lg:p-10 mb-6 bg-white rounded-lg shadow-lg"
                      x-ref="container{{ $i }}" :class="{ 'open': selected == {{ $i }} }">
                    <button type="button" class="w-full"
                      @click="selected !== {{ $i }} ? selected = {{ $i }} : selected = null">
                      @if (!empty($card['pill']))
                        <div class="rounded-full inline-block mb-4 px-3 py-1 bg-light text-action text-sm">{!! $card['pill'] !!}</div>
                      @endif
                    ...
                    <div class="relative overflow-hidden transition-all max-h-0 duration-700"
                      ...
                      <div class="text-lg border-t border-blue-300 mt-6 pt-6">{!! $card['description'] !!}</div>
                    </div>
                  </li>
                @endforeach
              @endisset
              ...
              <div class="flex items-start mt-6 gap-6">
                ...
                <div>
                  ...
                  <p class="text-brand text-base">
                    ...
                    @if (!empty($t['title'])),
                      <span class="text-action text-base font-semibold">{!! $t['title'] !!}</span>
                    @endif
                  </p>
                </div>
              </div>
