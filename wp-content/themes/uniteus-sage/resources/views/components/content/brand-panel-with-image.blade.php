...
<section @isset ($section['id']) id="{{ $section['id'] }}" @endisset class="component-section relative {{ $section_classes }} @if ($section_settings['collapse_padding']) {{ $section_settings['padding_class'] }} @endif">
  ...
  <div class="component-inner-section relative @if ($section_settings['fullscreen']) fullscreen @endif">
    <div class="bg-light w-full rounded-2xl flex flex-col md:relative md:flex-none md:grid md:grid-cols-2 lg:gap-20">
      ...
      <div class="@if ($featured_image) order-2 @endif p-9 md:p-20 md:pr-0 flex flex-col @if ('center' == $vertical_alignment) justify-center @endif text-lg @if ('text_image' == $layout) md:order-1 @else md:order-2 @endif lg:mb-0">
        @if ($section['subtitle'])
          <div class="subtitle n-case mb-3 lg:text-2xl">
            {{ $section['subtitle'] }}
          </div>
        @endif
        
        @if ($section['is_header'])
        <div class="mb-5 lg:text-4xl font-bold h2">{!! $section['title'] !!}</div>
        @else
        <h1 class="mb-5 lg:text-4xl font-bold h2">{!! $section['title'] !!}</h1>
        @endif
      ...
      <div class="relative p-9 md:p-9 @if ($featured_image) pb-0 md:p-0  @endif flex flex-col @if ('center' == $vertical_alignment) justify-center @endif @if ('text_image' == $layout) md:order-2 @else  md:order-1 @endif">
        @if ($featured_image)
