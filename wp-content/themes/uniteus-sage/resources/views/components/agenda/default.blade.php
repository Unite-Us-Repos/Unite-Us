    ...
    <div class="component-inner-section">
        ...
        <div class="flex items-center justify-center mx-auto">
            <div class="container mx-auto h-full flex flex-col justify-center items-stretch" x-data="{ tab: 0 }">
                ...
                <!-- TABS -->
                <div
                    class="flex flex-col items-center md:items-start md:flex-row text-xl justify-center md:gap-10 mb-4 z-10">
                    @if (count($tabs) > 1)
                        <span
                            class="text-brand font-medium md:py-4 no-underline font-medium inline-block align-middle">Explore
                            each day: </span>
                        <div class="flex gap-10">
                            @foreach ($tabs as $tab_index => $tab)
                                <a href="!#0" @click.prevent="tab = {{ $tab_index }}"
                                    :class="{ 'cursor-default border-b-2 border-action text-xl font-bold': tab ===
                                            {{ $tab_index }}, 'focus:outline-none focus:shadow-outline': tab !==
                                            {{ $tab_index }} }"
                                    class="py-4 no-underline font-medium inline-block align-middle">
                                    {{ $tab['title'] }}
                                </a>
                            @endforeach
                        </div>
                            ...
                            @if ($tab['sessions'])
                                @foreach ($tab['sessions'] as $s_index => $session)
                                    @if ($session)
                                        ...
                                        @endphp
                                        <div class="relative social-card py-6 px-9 lg:p-10 mb-6 bg-white rounded-lg shadow-lg"
                                            x-ref="container{{ $index . $s_index }}"
                                            :class="{ 'open': selected == {{ $index . $s_index }} }">

                                            <button type="button" class="w-full"
                                                @if ($session['description']) @click="selected !== {{ $index . $s_index }} ? selected = {{ $index . $s_index }} : selected = null" @else style="background: none;cursor: default" @endif>
                                                            ...
                                                            @else
                                                                <svg width="26" height="26" viewBox="0 0 26 26"
                                                                    ...
                                                                    <path
                                                                        ...
                                                                        stroke="#2874AF" stroke-width="2.14721"
                                            ...
                                            <div class="relative overflow-hidden transition-all max-h-0 duration-700"
                                                style="" x-ref="container{{ $index . $s_index }}"
                                                x-bind:style="selected == {{ $index . $s_index }} ? 'max-height: ' + $refs
                                                    .container{{ $index . $s_index }}.scrollHeight + 'px' : ''">
